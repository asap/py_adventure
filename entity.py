...
class Entity:
    ...
    def move_astar(self, target, entities, game_map):
        ...
        # Scan the current map each turn and set all the walls
        # as unwalkable
        ...
        # Scan all the objects to see if there are objects that
        # must be navigated around
        # Check also that the object isn't self or the target
        # (so that the start and the end points are free)
        # The AI class handles the situation if self is next to
        # the target so it will not use this A* function anyway
        ...
        # Check if the path exists, and in this case, also the
        # path is shorter than 25 tiles
        ...
        if (not libtcod.path_is_empty(my_path) and
            ...
            # a corridor)
            # it will still try to move towards the player (closer
            # to the corridor opening)
