...
def main():
    ...
    while not libtcod.console_is_window_closed():
        libtcod.sys_check_for_event(
            libtcod.EVENT_KEY_PRESS, key, mouse)
        ...
        render_all(
            con, entities, game_map, fov_map, fov_recompute,
            ...
            if not game_map.is_blocked(destination_x, destination_y):
                ...
                if target:
                    print(f"You bump into the {target.name}. " +
                          f"You can't understand  what it said, but " +
                          f"it wasn't pleasant.")
                else:
                    player.move(dx, dy)
                    fov_recompute = True
        ...
        if game_state == GameStates.ENEMY_TURN:
            for entity in entities:
                if entity != player:
                    print(f"The {entity.name} picks its nose")
